...
describe('Password Validator', () => {
  ...
  test('te kort wachtwoord geeft error', () => {
    ...
    expect(validatePassword('Test1').isValid).toBe(false); // Deze test faalt! Vervang met je eigen test
  });

  test('wachtwoord zonder nummer geeft error', () => {
    // TODO: Test of 'Welkomtestje' invalid is
    expect(validatePassword('Welkomtestje').isValid).toBe(false); // Deze test faalt! Vervang met je eigen test
  });

  test('wachtwoord zonder hoofdletter geeft error', () => {
    // TODO: Test of 'welkom123' invalid is
    expect(validatePassword('welkom123').isValid).toBe(false); // Deze test faalt! Vervang met je eigen test
  });
  ...
  test('errors array bevat juiste foutmeldingen', () => {
    ...
    expect(validatePassword('test').errors).toContain('Minimaal 8 karakters');
  });
