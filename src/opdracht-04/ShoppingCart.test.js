...
describe('ShoppingCart', () => {
  ...
  test('addItem met quantity voegt meerdere items toe', () => {
    cart.addItem({ id: 1, name: 'Product', price: 50 }, 3);
  ...
  test('removeItem verwijdert product', () => {
    cart.addItem({ id: 1, name: 'Product', price: 50 });
    cart.removeItem(1);
    expect(cart.getItemCount()).toBe(0);
  });

  test('clear maakt cart leeg', () => {
    cart.addItem({ id: 1, name: 'Product 1', price: 50 });
    cart.addItem({ id: 2, name: 'Product 2', price: 30 });
    cart.clear();
    expect(cart.getItemCount()).toBe(0);
  });
